...
//  YouTube
//
//  Created by md760 on 16.06.2018.
...
class HomeController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    ...
    func setupNavBarButtons() {
        ...
        let navBar = UIImage(named: "nav_more_icon")?.withRenderingMode(.alwaysOriginal)
        
        let searchBarButtonItem = UIBarButtonItem(image: searchImage, style: .plain, target: self, action: #selector(handleSearch))
        
        let moreButton = UIBarButtonItem(image: navBar, style: .plain, target: self, action: #selector(handleMore))
   ...
   private func setupMenuBar() {
    ...
    let redView = UIView()
    redView.backgroundColor = .rgb(red: 230, green: 32, blue: 31, alpha: 1)
